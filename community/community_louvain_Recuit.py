...
def best_partition_rs(graph, 
                        nbt,
                        nbi, t0, tt, argmax,
                        part_init=None, 
                        weight='weight', 
                        resolution=1.):
    """créer la partition en déroulant le recuit simulé classique
    """
    ...
    status_list = list()
    partition, nt = __recuit(nbt, nbi, graph, status, weight, resolution, t0, tt, argmax)
    status_list.append(partition)
    print('partition trouvée à la ' + str(nt) + 'ème itération')
    return partition
...
def best_partition_louvain_rs(graph,
                                nbt, nbi, t0, tt, argmax,
                                partition=None,
                                weight='weight',
                                resolution=1.,
                                randomize=None,
    ...
    """
    Compute the partition of the graph nodes which maximises the modularity
    (or try..) using the Louvain heuristices associated with a 'recuit-simulé' descent. 
    """
    dendo = generate_dendrogram_rs(graph,
                                nbt, nbi, t0, tt, argmax,
                                partition,
                                weight,
                                resolution,
...
def best_partition_louvain_then_rs(graph,
                                nbt, nbi, t0, tt, argmax,
                                partition=None,
                                weight='weight',
                                resolution=1.,
    ...
    dendo = generate_dendrogram(graph,
                                ...
                                randomize,
                                random_state)
    print('nombre de boucles louvain: ' + str(len(dendo)))
    current_graph = graph.copy()
    status = Status()
    status.init(current_graph, weight, partition_at_level(dendo, len(dendo) - 1))
    final_partition, nt = __recuit(nbt, nbi, current_graph, status, weight, resolution , t0, tt, partition_at_level(dendo, len(dendo) - 1),  random_state, argmax)
    print('final partition at iteration: ', str(nt))
...
def generate_dendrogram_rs(graph,
                            nbt, nbi, t0, tt, argmax,
                            part_init=None,
                            weight='weight',
                            resolution=1.,
                            randomize=None,
                            random_state=None):
    """Find communities in the graph and return the associated dendrogram, louvain+rs algorithm
    ...
    __one_level(current_graph, status, weight, resolution, random_state)
    partition = __renumber(status.node2com)
    partition, nt = __recuit(nbt, nbi, current_graph, status, weight, resolution, t0, tt,partition = partition, argmax=argmax)
    compteur=1
    print('nt = ' + str(nt))
    new_mod = __modularity(status, resolution)
    status_list.append(partition)
    ...
    while True:
        ...
        if new_mod - mod < __MIN:
            break
        nbi=10*len(graph.nodes())
        partition = __renumber(status.node2com)
        partition, nt = __recuit(nbt, nbi, current_graph, status, weight, resolution, t0, tt, partition=partition, argmax=argmax)
        print('compteur: ' + str(compteur) + '   nt=  ' +str(nt))
        current_graph = induced_graph(partition, current_graph, weight)
        status.init(current_graph, weight)
        new_mod = __modularity(status, resolution)
        print(new_mod)
...
def __one_level(graph, status, weight_key, resolution, random_state):
    ...
    while modified and nb_pass_done != __PASS_MAX:#à quoi sert nb pass done?
        cur_mod = new_mod
        modified = False
        nb_pass_done += 1

        for node in __randomize(graph.nodes(), random_state):#influence de changer randomize, pourquoi pas pour commencer sur les plus gros noeuds?
...
def __recuit(nbt, nbi, graph, status, weight_key, resolution, t0, tt, partition=None, random_state=None, argmax=False):
    if argmax:
        __recuitmax(nbt, nbi, graph, status, weight_key, resolution, t0, tt,random_state=None)
        return
    list_nodes = list(graph.nodes())
    curr_mod = __modularity(status, resolution)
    t=t0
    nb_com = max(set(status.node2com.values()))
    partition_best_l = partition
    nt=-1
    for nbtt in range(nbt):
        for nbii in range(nbi):
            last_mod = __modularity(status, resolution)
            ...
            incr = remove_cost + resolution * neighnode - \
                    status.degrees.get(neighcom, 0.) * degc_totw
            if incr > 0:
                __insert(node, neighcom,
                            neigh_communities.get(neighcom, 0.), status)
                if last_mod-curr_mod > __MIN:
                    partition_best_l = __renumber(status.node2com)
                    curr_mod= last_mod
                    print(last_mod)
    ...
    status.init(graph, 'weight', partition_best_l)
    return partition_best_l, nt

def __recuitmax(nbt, nbi, graph, status, weight_key, resolution, t0, tt,random_state=None):
    """Compute one level of communities, louvain+rs with argmax
    """
    list_nodes = list(graph.nodes())
    t=t0
...
def __insert(node, com, weight, status, graph = None ,nbcom=None):
    """ Insert node into community and modify status"""
    if nbcom != None:
        nbcom=+1
        status.node2com[node]=nbcom
        deg = float(graph.degree(node, weight=weight))
        if deg < 0:
            error = "Bad node degree ({})".format(deg)
            raise ValueError(error)
        status.degrees[nbcom] = deg
        status.gdegrees[node] = deg
        edge_data = graph.get_edge_data(node, node, default={weight: 0})
        status.loops[node] = float(edge_data.get(weight, 1))
        status.internals[nbcom] = status.loops[node]
        return nbcom
    else :
